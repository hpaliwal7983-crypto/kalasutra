...
function BuyerHomeScreen({ user, go, openProduct, wishlist, toggleWishlist, cartCount, addToCart, setToast }: any) {
  ...
  const craftItems = [
    ...
    ["Textiles", "/assets/textiles.png"],
    ["Woodwork", "/assets/woodwork.png"],
    ["Metalwork", "/assets/metalwork.png"],
    ["Cane & Bamboo", "/assets/cane.png"],
    ["Jewellery", "/assets/jewellery.png"],
    ["Home Decor", "/assets/home-decor.png"],
  ];

  const demoProducts = [
    { title: "Handwoven Cane Dome Pendant Lamp", maker: "Radha Devi", region: "Jaipur, Rajasthan", price: 1499, image: "/assets/cane.png", category: "Cane & Bamboo" },
    { title: "Terracotta Minimal Vase", maker: "Suresh Prajapat", region: "Jaipur, Rajasthan", price: 899, image: "/assets/pottery.png", category: "Pottery" },
    { title: "Handcarved Wooden Serving Bowl", maker: "Mohan Lal", region: "Udaipur, Rajasthan", price: 1200, image: "/assets/woodwork.png", category: "Woodwork" },
    { title: "Block Print Cushion Cover", maker: "Fatima Bano", region: "Sanganer, Rajasthan", price: 699, image: "/assets/textiles.png", category: "Textiles" },
  ];

  const visibleProducts = filtered.length ? filtered.slice(0, 4) : demoProducts;
  ...
  function chooseCraft(name: string) {
    setQuery(name === "Home Decor" ? "decor" : name);
    window.setTimeout(() => window.scrollTo({ top: 720, behavior: "smooth" }), 20);
  }

  function productIdFor(p:any) {
    return p.id || products.find(x => x.title === p.title)?.id || null;
  }

  return <div className="buyer-reference-home">
    <ErrorBanner message={err}/>

    <section className="buyer-ref-hero">
      <div className="buyer-ref-topline">
        <div className="buyer-brand-block">
          <img src="/assets/logo.png" alt="KalaSutra" className="buyer-ref-logo"/>
          <div className="buyer-ref-artline">Art<br/>Lives<br/>Here ♡</div>
        </div>
        <div className="buyer-ref-actions">
          <button aria-label="Search" onClick={() => document.getElementById("buyer-smart-search")?.focus()}><Icon name="search"/></button>
          <button aria-label="Notifications"><Icon name="bell"/></button>
          <button aria-label="Settings">⚙</button>
        </div>
      </div>
      <div className="buyer-ref-hero-copy">
        <div className="buyer-ref-copy-left">
          <h1>Hello, {user.name} <span>✦</span></h1>
          <p>Discover something made by hand,<br/>made with a story.</p>
          <button className="buyer-ref-cta" onClick={() => window.scrollTo({top: 430, behavior:"smooth"})}>Explore Handmade <b>→</b></button>
        </div>
        <div className="buyer-ref-hero-photo" aria-label="People Crafts Stories">
          <img src="/assets/hero-photo.png" alt="People Crafts Stories"/>
        </div>
      </div>
    </section>

    <section className="buyer-ref-search-area">
      <div className="buyer-ref-search">
        <Icon name="search"/>
        <input id="buyer-smart-search" value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search crafts, materials, makers..."/>
        <button className={listening ? "buyer-ref-mic listening" : "buyer-ref-mic"} onClick={voiceSearch} aria-label="Voice search">🎙</button>
      </div>
      <div className="buyer-ref-shortcuts">
        <button onClick={()=>go("wishlist")}><Icon name="heart"/> Saved</button>
        <button onClick={()=>go("buyerReels")}><Icon name="reels"/> Maker Reels</button>
        <button onClick={()=>go("cart")}><Icon name="cart"/> Cart</button>
      </div>
    </section>

    <section className="buyer-ref-section buyer-ref-crafts">
      <div className="buyer-ref-section-head"><div>EXPLORE BY CRAFT</div><button onClick={()=>setQuery("")}>See all →</button></div>
      <div className="buyer-craft-scroller">
        {craftItems.map(([name,img]) => <button className="buyer-craft-item" key={name} onClick={()=>chooseCraft(name)}>
          <span><img src={img} alt={name}/></span><b>{name}</b>
        </button>)}
        <button className="buyer-craft-item buyer-more-craft" onClick={()=>go("buyerReels")}><span>›</span><b>More</b></button>
      </div>
    </section>

    <section className="buyer-today-story">
      <div className="buyer-today-copy">
        <div className="buyer-ref-eyebrow">TODAY’S CRAFT STORY</div>
        <h2>From Jaipur, with hands<br/>that have carried a<br/>tradition forward.</h2>
        <p>Meet Radha Devi, a cane craft artisan from Jaipur who turns simple materials into timeless pieces.</p>
        <button onClick={()=>go("buyerReels")}>View Story <b>→</b></button>
      </div>
      <div className="buyer-today-photo"><img src="/assets/todays-story.png" alt="Jaipur artisan"/><div className="buyer-story-quote">“Every weave tells a story of resilience.”<small>— Radha Devi<br/><span>⌖ Jaipur, Rajasthan</span></small></div><span className="buyer-verified-pill">● Verified Artisan</span></div>
    </section>

    <section className="buyer-ref-section buyer-curated">
      <div className="buyer-ref-section-head"><div>CURATED FOR YOU <small>Handpicked pieces from verified artisans.</small></div><button onClick={()=>setQuery("")}>See all →</button></div>
      <div className="buyer-curated-grid">
        {visibleProducts.map((p:any,i:number)=>{
          const pid=productIdFor(p);
          return <article className="buyer-curated-card" key={p.id || p.title} onClick={()=>pid && openProduct(pid)}>
            <div className="buyer-curated-img"><img src={p.image || "/assets/pottery.png"} alt={p.title}/><span className="buyer-verified-tag">● Verified Handmade</span><button className="buyer-heart" onClick={e=>{e.stopPropagation(); if(pid) toggleWishlist(pid)}}>{pid && wishlist.includes(pid) ? "♥" : "♡"}</button></div>
            <div className="buyer-curated-info"><h3>{p.title}</h3><p>by {p.maker || p.artisanName || "KalaSutra Artisan"}<br/>{p.region || p.craftInfo?.region || "Rajasthan"}</p><div><strong>₹{Number(p.price||0).toLocaleString("en-IN")}</strong><button onClick={e=>{e.stopPropagation(); if(pid) { addToCart(pid); setToast("Added to cart 🛍️"); }}}>＋ Add to cart</button></div></div>
          </article>
        })}
      </div>
    </section>

    <section className="buyer-why-handmade">
      <div className="buyer-why-title"><b>WHY BUY HANDMADE?</b><span>More than products — a better tomorrow.</span></div>
      <div className="buyer-why-items">
        <div><i>♜</i><b>Verified<br/>Handmade</b><small>Craft authenticity checked.</small></div>
        <div><i>₹</i><b>Fair Price</b><small>Help makers earn fairly.</small></div>
        <div><i>♟</i><b>Meet the Maker</b><small>Know the person behind your piece.</small></div>
        <div><i>◒</i><b>Craft Legacy</b><small>Every purchase keeps traditions alive.</small></div>
      </div>
    </section>
