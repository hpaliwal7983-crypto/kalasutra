...
function BuyerNav({ screen, go, cartCount }) {
    ...
    return (React.createElement("div", { className: "bottom-nav buyer-bottom-nav buyer-home-nav" }, items.map((it) => (React.createElement("button", { key: it.id, className: `nav-btn ${screen === it.id ? "active" : ""}`, onClick: () => go(it.id) },
        React.createElement("span", { className: "nav-icon-wrap" }, React.createElement(Icon, { name: it.icon })),
        React.createElement("span", null, it.label))))));
}
// ---------------------------------------------------------------------------
// BUYER: HOME / EXPLORE
// ---------------------------------------------------------------------------
function BuyerHomeScreen({ user, go, openProduct, wishlist, toggleWishlist, cartCount }) {
    const [products, setProducts] = useState([]);
    ...
    const categoryTiles = categories.map(item => e("button", {
        key: item.label,
        className: `buyer-home-category ${category === item.label ? "active" : ""}`,
        onClick: () => item.label === "All Categories" ? resetFilters() : setCategory(category === item.label ? "" : item.label),
        "aria-pressed": category === item.label,
    }, e("span", null, item.image ? e("img", { src: item.image, alt: "", loading: "lazy" }) : e("b", { className: "buyer-all-categories-icon", "aria-hidden": "true" }, "▦")), e("small", null, item.label)));
    const productCards = filtered.map(product => e("article", {
        key: product.id,
        className: "buyer-home-product-card",
        onClick: () => openProduct(product.id),
    },
        e("div", { className: "buyer-home-product-image", style: { backgroundImage: `url(${product.image || ""})` } },
            e(BadgeLabel, { status: product.verificationStatus }),
            e("button", {
                className: "buyer-home-heart",
        ...
        e("div", { className: "buyer-home-product-info" },
            e("div", { className: "buyer-home-product-title" }, product.title),
            e("small", { className: "buyer-home-rating" }, `★ ${Number(product.rating || 4.8).toFixed(1)} (${product.reviewCount || 98})`),
            e("strong", { className: "buyer-home-price" }, `₹${Number(product.price || 0).toLocaleString("en-IN")}`))));
    ...
    return e(React.Fragment, null,
        e("main", { className: "buyer-home-reference" },
            e("header", { className: "buyer-home-topbar" },
                e("button", { className: "buyer-home-menu-button", onClick: () => setMenuOpen(open => !open), "aria-label": menuOpen ? "Close menu" : "Open menu", "aria-expanded": menuOpen }, e("i"), e("i"), e("i")),
                e("div", { className: "buyer-home-brand", "aria-label": "KalaSutra — Crafts Connect Hearts" }, e("span", { className: "buyer-home-brand-leaf", "aria-hidden": "true" }, "❧"), e("span", null, e("strong", null, "KalaSutra"), e("small", null, "— Crafts Connect Hearts —"))),
            e("div", { className: "buyer-home-actions" },
                    e("button", { onClick: () => go("wishlist"), "aria-label": `Wishlist${wishlist.length ? `, ${wishlist.length} saved` : ""}` }, e(Icon, { name: "heart" }), wishlist.length > 0 && e("b", { className: "buyer-home-count" }, wishlist.length > 9 ? "9+" : wishlist.length)),
                    e("button", { onClick: () => go("cart"), "aria-label": `Cart${cartCount ? `, ${cartCount} items` : ""}` }, e(Icon, { name: "orders" }), cartCount > 0 && e("b", { className: "buyer-home-count" }, cartCount > 9 ? "9+" : cartCount)),
                    e("button", { className: "buyer-home-profile", onClick: () => go("buyerProfile"), "aria-label": "Open profile" }, e("img", { src: user.profile?.photo || "/assets/avatar-artisan.png", alt: "" }))),
                menuOpen && e("nav", { className: "buyer-home-menu", "aria-label": "Buyer menu" }, menuLinks.map(item => e("button", { key: item.id, onClick: () => { setMenuOpen(false); go(item.id); } }, item.label)))),
            e("div", { className: "buyer-home-search" }, e(Icon, { name: "search" }), e("input", { value: query, onChange: event => setQuery(event.target.value), placeholder: "Search for handcrafted treasures…", "aria-label": "Search handmade products" }), e("button", { className: listening ? "listening" : "", onClick: voiceSearch, "aria-label": listening ? "Listening" : "Voice search" }, "🎙")),
            e("section", { className: "buyer-home-hero" }, e("div", { className: "buyer-home-hero-copy" }, e("span", null, "DISCOVER"), e("h1", null, "Handmade with Heart"), e("p", null, "Support real artisans. Bring home unique stories."), e("button", { onClick: () => document.getElementById("buyer-featured-picks")?.scrollIntoView({ behavior: "smooth", block: "start" }) }, "Explore Now", e("b", null, "→"))), e("img", { className: "buyer-home-hero-image", src: "/buyer-story.jpg", alt: "An artisan handweaving a handmade craft" })),
            e("nav", { className: "buyer-home-categories", "aria-label": "Browse craft categories" }, categoryTiles),
            e("section", { id: "buyer-featured-picks", className: "buyer-home-featured" }, e("div", { className: "buyer-home-section-heading" }, e("h2", null, query || category ? "Matching Crafts" : "Featured Picks"), e("button", { onClick: resetFilters }, "View All", e("span", null, "→"))),
                e(ErrorBanner, { message: err }), filtered.length === 0 ? e("div", { className: "empty-note" }, products.length ? "No pieces match your search — try another craft or material." : "No handmade pieces are available right now.") : e("div", { className: "buyer-home-product-grid" }, productCards)),
            e("section", { className: "buyer-home-trust", "aria-label": "Shopping benefits" },
                e("div", null, e("span", null, "❧"), e("p", null, e("strong", null, "100% Authentic"), e("small", null, "Real Artisans"))),
                e("div", null, e("span", null, "♢"), e("p", null, e("strong", null, "Secure Payments"), e("small", null, "Shop with Confidence"))),
                e("div", null, e("span", null, "♧"), e("p", null, e("strong", null, "Fast Delivery"), e("small", null, "Across India"))),
                e("div", null, e("span", null, "♡"), e("p", null, e("strong", null, "Support Crafts"), e("small", null, "Make a Difference"))))));
}
...
function App() {
    ...
    const navBar = isArtisan
        ...
        : ["buyerHome", "buyerReels", "cart", "orders", "wishlist", "buyerProfile"].includes(screen) && React.createElement(BuyerNav, { screen: screen, go: setScreen, cartCount: cartCount });
