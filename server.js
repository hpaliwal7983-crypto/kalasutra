...
async function api(req,res,url){
  ...
  try{
    if(resource==='ai' && parts[2]==='realtime' && req.method==='POST'){
      const key=process.env.OPENAI_API_KEY;
      if(!key) return json(res,503,{error:'OPENAI_API_KEY is not configured on the server'});
      const sdp=await rawBody(req,2*1024*1024);
      if(!sdp) return json(res,400,{error:'SDP offer is required'});
      const role=String(url.searchParams.get('role')||'artisan');
      const language=String(url.searchParams.get('language')||'hi');
      ...
      const label=langNames[language]||'Hindi';
      const instructions=[
        'You are Karigar AI, the warm voice-first copilot inside KalaSutra.',
        `Role: ${role}. User name: ${user || 'not provided'}. Preferred language: ${label}. Current screen: ${screen || 'unknown'}.`,
        'Speak naturally like a warm, capable friend. Short turns. Human rhythm. Gentle pauses. No robotic IVR, no formal script, no long lists.',
        'Understand Hindi, English, Hinglish, and the selected Indian language. Reply in the user\'s language unless they clearly switch.',
        'The user should feel like they are talking to an AI companion, not a chatbot form.',
        'For casual questions, converse naturally and ask one helpful follow-up.',
        'When the user explicitly asks to open or do an app action, use navigate_app. For Add Product, guide one step at a time through photos, story, details, fair price, making-proof video, verification, and publish confirmation.',
        'Never claim an app action is complete until the app actually navigates or completes it.'
      ].join(' ');
      const sessionConfig=JSON.stringify({
        ...
        tools:[{type:'function',name:'navigate_app',description:'Open a real KalaSutra screen when the user explicitly asks to do so.',parameters:{type:'object',properties:{screen:{type:'string',enum:['dashboard','addProduct','orders','createReel','profile','buyerHome','wishlist','cart','myProducts','myReels']}},required:['screen']}}]
      });
      const form=new FormData();
      form.set('sdp',sdp);
      form.set('session',sessionConfig);
      const safety=crypto.createHash('sha256').update(`${user||'anonymous'}|kalasutra`).digest('hex');
      const rr=await fetch('https://api.openai.com/v1/realtime/calls',{method:'POST',headers:{'Authorization':`Bearer ${key}`,'OpenAI-Safety-Identifier':safety},body:form});
      const answer=await rr.text();
      if(!rr.ok){return json(res,502,{error:`OpenAI realtime session failed (${rr.status})`,detail:answer.slice(0,800)})}
      res.writeHead(200,{'Content-Type':'application/sdp','Cache-Control':'no-store','Access-Control-Allow-Origin':'*'});
    ...
    if(resource==='ai' && parts[2]==='chat' && req.method==='POST'){
      ...
      const payload={model:process.env.OPENAI_MODEL||'gpt-5.5',input:[{role:'system',content:system},...messages.map(m=>({role:m.role,content:String(m.content||'')}))],max_output_tokens:220};
