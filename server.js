...
// ---------- Karigar AI (server-side, keeps the API key off the browser) ----------
    ...
    const r = https.request({
      ...
      rr.on('end', () => {
        ...
        if (rr.statusCode < 200 || rr.statusCode >= 300) {
          ...
          return reject(new Error(msg || `OpenAI returned ${rr.statusCode}`));
        }
...
function karigarFallback(message, role) {
  ...
  const replies = {
    addProduct: 'Bilkul. Chalo naya product add karte hain. Pehle ek achhi photo lete hain.',
    orders: 'Haan, chalo orders dekhte hain.',
    createReel: 'Chalo is product ki Reel banate hain.',
    fairPrice: 'Chalo is product ki fair price dekhte hain.',
    materialHub: 'Chalo raw material ke liye Material Hub kholte hain.',
    marketMatch: 'Chalo suitable buyers ke liye Market Match dekhte hain.',
    designLab: 'Chalo Design Lab mein fresh ideas dekhte hain.',
    craftPassport: 'Chalo aapka Craft Passport kholte hain.',
    craftGurukul: 'Chalo Craft Gurukul kholte hain.',
    profile: 'Chalo profile kholte hain.',
    buyerProfile: 'Chalo aapka buyer profile kholte hain.',
    dashboard: 'Chalo KalaSutra home par chalte hain.',
    buyerHome: 'Chalo KalaSutra home par chalte hain.',
    cart: 'Chalo cart kholte hain.',
    wishlist: 'Chalo saved pieces dekhte hain.'
  };
...
async function handleAPI(req, res, urlParts) {
  ...
  try {
    // ----- KARIGAR AI -----
    if (resource === 'karigar-ai' && !id && req.method === 'POST') {
      const body = await readBody(req);
      const role = body.role === 'artisan' ? 'artisan' : 'buyer';
      const language = String(body.language || 'hi-IN');
      const allowedActions = ['addProduct','orders','createReel','profile','buyerProfile','dashboard','buyerHome','cart','wishlist','fairPrice','materialHub','marketMatch','designLab','craftPassport','craftGurukul'];
      const fallback = karigarFallback(body.message, role);
      if (!process.env.OPENAI_API_KEY) return sendJSON(res, 200, fallback);

      const system = `You are Karigar AI inside KalaSutra, an Indian artisan marketplace. You are an AI companion, not a human. Talk like a warm, modern, helpful friend: short natural turns, gentle pauses, varied emphasis, no formal scripts, no long lectures. Respond in the user's selected language (${language}) while naturally understanding Hindi-English mixed speech. Never invent orders, earnings, buyers, prices, or app data. When a task is clearly requested, return a compact action along with the reply. The user wants voice-first interaction with no chatty UI.
Return ONLY valid JSON in this exact shape: {"reply":"...","action":"..."}. action must be one of ${JSON.stringify(allowedActions)} or null.`;
      const recent = Array.isArray(body.history) ? body.history.slice(-8) : [];
      const context = JSON.stringify(body.context || {});
      const transcript = recent.map(x => `${x.role === 'assistant' ? 'AI' : 'USER'}: ${String(x.content || '').slice(0,500)}`).join('\n');
      const input = `Context: ${context}\nRecent conversation:\n${transcript}\nUSER: ${String(body.message || '').slice(0,1200)}`;
      try {
        const out = await openAIRequest('/v1/responses', {
          model: process.env.KARIGAR_AI_MODEL || 'gpt-5.6-luna',
          instructions: system,
          input,
          max_output_tokens: 220,
        });
        const raw = cleanJsonText(extractResponseText(out));
        let parsed;
        try { parsed = JSON.parse(raw); } catch (_) { parsed = null; }
        const reply = String(parsed?.reply || '').trim();
        const action = allowedActions.includes(parsed?.action) ? parsed.action : (parsed?.action === null ? null : fallback.action);
        return sendJSON(res, 200, { reply: reply || fallback.reply, action });
      } catch (e) {
        console.error('Karigar AI fallback:', e.message);
        return sendJSON(res, 200, fallback);
      }
    }

    if (resource === 'karigar-ai' && id === 'tts' && req.method === 'POST') {
      const body = await readBody(req);
      const text = String(body.text || '').trim().slice(0, 1200);
      if (!text) return sendJSON(res, 400, { error: 'Text is required' });
      if (!process.env.OPENAI_API_KEY) return sendJSON(res, 200, { audio: null, mime: 'audio/mpeg' });
      try {
        const audio = await openAIRequest('/v1/audio/speech', {
          model: process.env.KARIGAR_AI_TTS_MODEL || 'gpt-4o-mini-tts',
          voice: process.env.KARIGAR_AI_VOICE || 'alloy',
          input: text,
          instructions: 'Speak warmly and naturally, like a friendly AI companion helping an artisan. Keep the pacing conversational and human-like, with gentle pauses and natural emphasis. Do not sound like a formal announcement.',
          response_format: 'mp3',
        }, 'buffer');
        return sendJSON(res, 200, { audio: audio.toString('base64'), mime: 'audio/mpeg' });
      } catch (e) {
        console.error('Karigar TTS fallback:', e.message);
        return sendJSON(res, 200, { audio: null, mime: 'audio/mpeg' });
      }
    }
