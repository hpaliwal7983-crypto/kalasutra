...
function AITalker({ compact = false, embedded = false, role = 'buyer', go, userName = '' }) {
    ...
    // iPhone/iPad Safari only lets audio play if the <audio> element was first
    // "unlocked" inside a real tap. We unlock ONE shared element on every tap and
    // reuse it for all AI speech, so voice keeps working after async network calls.
    function unlockAudio() {
        try {
            ...
            if (el.dataset && el.dataset.unlocked === '1') return;
            el.src = SILENT_WAV;
            const p = el.play();
            if (p && p.then) {
                p.then(() => {
                    if (el.dataset) el.dataset.unlocked = '1';
                    if (el.src === SILENT_WAV) { try { el.pause(); } catch (_) {} }
                }).catch(() => {});
            } else if (el.dataset) {
    ...
    async function speak(text) {
        ...
        try { sharedAudioRef.current && sharedAudioRef.current.pause(); } catch (_) {}
        ...
        try {
            ...
            if (r.ok) {
                ...
                const played = await new Promise(resolve => {
                    ...
                    audio.src = url;
                    const p = audio.play();
                    if (p && p.catch) p.catch(err => { console.warn('Audio play blocked:', err); finish(false); });
                });
                ...
                console.warn('TTS request failed with status', r.status);
                setAiError('Voice server error (' + r.status + ')' + (r.status === 503 ? ' — OPENAI_API_KEY Render mein set karo' : ''));
            }
        } catch (err) { console.warn('TTS error', err); }

        // Fallback: the browser's own voice (more robotic). May also need a tap on iPhone/iPad.
        try {
            if ('speechSynthesis' in window && typeof SpeechSynthesisUtterance !== 'undefined') {
                const ok = await new Promise(resolve => {
                    ...
                    window.setTimeout(() => { if (!started) resolve(false); }, 2500);
                });
                if (ok) { if (mountedRef.current) setSpeaking(false); return true; }
            }
        } catch (_) {}
    ...
    async function startListening({ keepConversation = false } = {}) {
        ...
        const SR = window.SpeechRecognition || window.webkitSpeechRecognition;
        // On iPad/iPhone, prefer actual microphone recording + server transcription.
        // This avoids Safari's intermittent SpeechRecognition "Listening…" state
        // where no transcript is delivered.
    ...
    async function askAI(text) {
        ...
        try {
            ...
            console.error('Karigar AI chat failed:', err);
            setAiError('AI server error: ' + String((err && err.message) || err));
            setThinking(false);
    ...
    const aiPanel = React.createElement('div', {
        ...
        React.createElement('div', { className: 'ai-talker-body' },
            ...
            audioUnlockNeeded && React.createElement('button', {
                className: 'ai-lang-pill',
                style: { margin: '6px 0' },
                onClick: () => { unlockAudio(); const t = pendingSpeechRef.current || message; speak(t); }
            }, '🔊 Tap to hear'),
            aiError && React.createElement('div', { className: 'ai-message', style: { fontSize: 12, color: '#b3261e' } }, '⚠ ' + aiError),
